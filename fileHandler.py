...
def addFileToFrame(frame,row,col, name, format):
    file_path = os.path.join(getStaticDownPath(),f"{name}.mp4")
    if os.path.exists(file_path) == False:
        file_path = os.path.join(getStaticDownPath(),f"{name}.mp3")
...
def filePosition():
    ...
    try:
        Files.remove(directory + "\\plt.txt")
    except:
        pass
    numFiles = len(Files)
    return ((numFiles-1)//3,(numFiles-1)%3)

def refreshFilelog():
    file_log_path = getStaticLogpath("fileLog.txt")
    download_path = getStaticDownPath()
    pattern = os.path.join(download_path, '*.*')
    Files = glob.glob(pattern)
    try:
        Files.remove(download_path + "\\plt.txt")
    except:
        pass
    logfile = "[DownloadedFilesDoc:\]"
    if len(Files) != 0:
        logfile += ";"
        for i in range(len(Files)):
            Files[i] = Files[i].split("\\")[len(Files[i].split("\\"))-1].split(".")[0]
            if i != len(Files) -1:
                logfile += Files[i] + ";"
            else:
                logfile += Files[i]
    with open(file_log_path, "w") as log:
        log.write(logfile)
        log.close()
...
def deleteAll(delproof, frame):
    ...
    directory = getStaticDownPath()
    mp4pattern = os.path.join(directory,"*.mp4")
    mp3pattern = os.path.join(directory,"*.mp3")
